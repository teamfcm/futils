...
namespace futils
{
    class   INI
    {
        ...
        bool            isSectionString(std::string const &ref)
        {
            std::string cpy{this->epur(ref)};
            return (cpy[0] == '[' && cpy[cpy.size() - 1] == ']');
        }

        std::string     extractSectionName(std::string const &ref)
        {
            std::string cpy{this->epur(ref)};
            return cpy.substr(1, cpy.size() - 2);
        }

        std::string     extractTokenName(std::string const &ref)
        {
            auto    delimPos = ref.find("=");
            return ref.substr(0, delimPos);
        }

        std::string     extractTokenValue(std::string const &ref)
        {
            auto    delimPos = ref.find("=");
            return ref.substr(delimPos + 1, ref.size());
        }

        bool            isToken(std::string const &ref)
        {
            auto index = ref.find("=");
        ...
        std::string     epur(std::string const &str)
        {
            std::string cleanStr;
        ...
        void    writeContentTo(std::string const &file)
        {
